...
def normalizar_tags(dados_exif):
    """Normaliza as tags dos metadados EXIF conforme o dicionário TAGS_MAP."""
    return {TAGS_MAP.get(tag, tag): value for tag, value in dados_exif.items()}
...
def main():
    ...
    for nome_arquivo, dados in dados_exif.items():
        dados_normalizados = [normalizar_tags(d) for d in dados]
        dados_com_caminho = [{**d, 'caminho_imagem': foto} for d, foto in zip(dados_normalizados, fotos)]
